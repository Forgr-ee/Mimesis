...
const service_account = readFileSync('./seed_db/firebase_admin.json',{encoding:'utf8', flag:'r'})
    ...
    try {
        await new Promise((resolve, reject) => {
            get(url, (response) => {
                ...
                .on('error', (error) => reject(`error ${url}`))
            })
        })
    } catch (err: any) {
...
const parseAndUpload = async (folder: string, kind: string) => {
    const ref = firestore.collection(`mode/${folder}${kind === 'base' ? '' : '_plus'}/fr`)
    console.log('Load data csv', folder)
    const datas = await csvtojson({delimiter: ';'}).fromFile(`./seed_db/${folder}/${kind}.csv`)
    ...
    for (let index = 0; index < datas.length; index++) {
        ...
        if (data.cover) {
            ...
            try {
                ...
                const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${storageBucket}/o/${encodeURIComponent(destination)}?alt=media`
